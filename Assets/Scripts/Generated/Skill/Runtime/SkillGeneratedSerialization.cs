...
namespace Hoshino
{
    ...
    public sealed class SkillGeneratedRuntimeSerialization : ISkillGeneratedRuntimeSerialization
    {
        ...
        public void Preload(SkillDefinition skill)
        {
            SkillGeneratedNodeDataBlob.Preload(skill);
        }
    }
    ...
    public static class SkillGeneratedNodeDataBlob
    {
        ...
        public static SkillGeneratedPreloadedData Preload(SkillDefinition skill)
        {
            SkillGeneratedPreloadedData data = new SkillGeneratedPreloadedData();
            if (skill == null)
                return data;
            ...
            for (int i = 0; i < nodes.Length; i++)
            {
                ...
                if (node.DataLength <= 0 || !IsClipKnown(node.ClipId))
                    continue;
                if (blob == null || node.DataOffset < 0 || (long)node.DataOffset + node.DataLength > blob.Length)
                    continue;
                using MemoryStream stream = new(blob, node.DataOffset, node.DataLength, false);
                using BinaryReader reader = new(stream);
                try
                {
                    ...
                    data.Add(node.ClipId, node.NodeId, value);
                }
                catch (Exception)
                {
                }
            }
            skill.PreloadedNodeData = data;
            return data;
        }
    }

    public sealed class SkillGeneratedPreloadedData
        : ISkillPreloadedNodeData<SetVelocityNodeData>,
          ...
          ISkillPreloadedNodeData<MultiDamageNodeData>
    {
        ...
        bool ISkillPreloadedNodeData<TeleportNodeData>.TryGetValue(int nodeId, out TeleportNodeData data)
            => _TeleportClip.TryGetValue(nodeId, out data);

        bool ISkillPreloadedNodeData<AttributeModifierNodeData>.TryGetValue(int nodeId, out AttributeModifierNodeData data)
            => _AttributeModifierClip.TryGetValue(nodeId, out data);

        bool ISkillPreloadedNodeData<SingleDamageNodeData>.TryGetValue(int nodeId, out SingleDamageNodeData data)
            => _SingleDamageClip.TryGetValue(nodeId, out data);
        ...
        internal void Add(uint clipId, int nodeId, object value)
        {
            ...
            {
                case SkillGeneratedIds.SetVelocityClip:
                    _SetVelocityClip[nodeId] = (SetVelocityNodeData)value;
                    break;
                case SkillGeneratedIds.TeleportClip:
                    _TeleportClip[nodeId] = (TeleportNodeData)value;
                    break;
                case SkillGeneratedIds.AttributeModifierClip:
                    _AttributeModifierClip[nodeId] = (AttributeModifierNodeData)value;
                    break;
                case SkillGeneratedIds.SingleDamageClip:
                    _SingleDamageClip[nodeId] = (SingleDamageNodeData)value;
                    break;
                case SkillGeneratedIds.MultiDamageClip:
